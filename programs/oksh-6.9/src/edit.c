    ...
    int *nwords, char ***words)
{
	...
	*words = NULL;
	...
	while (cmd < want && isspace((uint8_t)*cmd))
		cmd++;
	cmdlen = 0;
	while (cmd + cmdlen < want && !isspace((uint8_t)cmd[cmdlen]))
		cmdlen++;
	for (i = 0; i < cmdlen; i++) {
		if (!isalnum((uint8_t)cmd[i]) && cmd[i] != '_')
			return 0;
	}
	...
	for (cp = cmd + cmdlen + 1; cp < want; cp++) {
		if (!isspace((uint8_t)cp[-1]) && isspace((uint8_t)*cp))
			n++;
	}
